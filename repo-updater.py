...
def copy_and_backup_changed_files(src_dir, dst_dir):
    old_dir = os.path.join(dst_dir, ".old")
    os.makedirs(old_dir, exist_ok=True)

    for root, dirs, files in os.walk(src_dir):
        rel_path = os.path.relpath(root, src_dir)
        target_root = os.path.join(dst_dir, rel_path)
        old_root = os.path.join(old_dir, rel_path)
        ...
        for file in files:
            ...
            if files_differ(src_file, dst_file):
                if os.path.exists(dst_file):
                    ...
                    shutil.move(dst_file, os.path.join(old_root, file))
                    print(f"Backed up: {os.path.relpath(dst_file, dst_dir)}")

                os.makedirs(target_root, exist_ok=True)
                shutil.copy2(src_file, dst_file)
                print(f"Updated: {os.path.relpath(dst_file, dst_dir)}")

def delete_old_folder(dst_dir):
    old_dir = os.path.join(dst_dir, ".old")
...
def main():
    ...
    copy_and_backup_changed_files(repo_dir, local_dir)

    os.remove(zip_path)
    shutil.rmtree(os.path.dirname(repo_dir))

    print("Deleting .old folder...")
    delete_old_folder(local_dir)
